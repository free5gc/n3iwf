...
import (
	...
	"free5gc/src/n3iwf/handler/message"
...
func Send(sendInfo *message.UDPSendInfoGroup, msg []byte) {
...
func reader(channelID int, conn *net.UDPConn) {
	...
	for {
		...
		sendInfo := &message.UDPSendInfoGroup{
			ChannelID: channelID,
			Addr:      remoteAddr,
		}

		msg := message.HandlerMessage{
			Event:       message.EventN1UDPMessage,
